...
try {
  ...
  const paths = [...sitemapXml.matchAll(/<loc>https:\/\/www\.chinafreeweight\.com(\/pl(?:\/[^<]*)?)<\/loc>/g)].map((match) => match[1]);
  ...
  for (const path of paths) {
    ...
    assert.match(html, /<html lang="pl" dir="ltr">/, `${path}: lang`);
    ...
    assert.equal(await page.locator('link[hreflang="pl"]').getAttribute("href"), `${siteUrl}${path}`, `${path}: hreflang`);
    ...
    assert.ok(nodes.some((node) => node?.["@type"] === "BreadcrumbList" && node.inLanguage === "pl"), `${path}: BreadcrumbList`);
    assert.ok(nodes.some((node) => node?.["@type"] === "FAQPage" && node.inLanguage === "pl"), `${path}: FAQPage`);
    if (/^\/pl\/produkty\/[^/]+\/[^/]+$/.test(path)) {
      assert.ok(nodes.some((node) => node?.["@type"] === "Product" && node.inLanguage === "pl"), `${path}: Product`);
      assert.equal(await page.locator('main[data-page-family="product-detail"]').count(), 1, `${path}: product family`);
      assert.ok(await page.locator('main[data-page-family="product-detail"] img[src^="/assets/products/"]').count() >= 1, `${path}: English product asset`);
    }
    if (/^\/pl\/blog\/[^/]+$/.test(path)) assert.ok(nodes.some((node) => node?.["@type"] === "BlogPosting" && node.inLanguage === "pl"), `${path}: BlogPosting`);
    ...
    assert.equal(await page.locator('.route-language-switcher--desktop a[lang="pl"]').count(), 1, `${path}: Polish switch option`);
  }

  await page.goto(testUrl("/pl/produkty"), { waitUntil: "domcontentloaded" });
  ...
  await page.goto(testUrl("/pl/kontakt"), { waitUntil: "networkidle" });
  ...
  assert.equal(await page.locator('form.quote-form input[name="_subject"]').getAttribute("value"), "Nowe zapytanie B2B w języku polskim — ChinaFreeWeight");
  assert.equal(await page.locator('form.quote-form input[name="_next"]').getAttribute("value"), "https://www.chinafreeweight.com/pl/kontakt?inquiry=sent#zapytanie");
  assert.match(await page.locator(".whatsapp-button").getAttribute("href"), /^https:\/\/wa\.me\/8618963018533/);

  await page.goto(testUrl("/pl"), { waitUntil: "domcontentloaded" });
  ...
  assert.equal((languageXml.match(/<loc>https:\/\/www\.chinafreeweight\.com\/pl(?:<|\/)/g) ?? []).length, 122);
  const robots = await page.goto(testUrl("/robots.txt"), { waitUntil: "domcontentloaded" });
  assert.doesNotMatch(await robots.text(), /^Disallow:\s*\/pl(?:\/|$)/im);
  const missing = await page.goto(testUrl("/pl/produkty/nieistniejacy-produkt"), { waitUntil: "domcontentloaded" });
  assert.equal(missing?.status(), 404);
  console.log(`Polish browser smoke passed: ${paths.length} SSR pages, assets, schemas, forms, sitemaps, robots and 404.`);
} finally {
  await browser.close();
}
