...
test("published locale routing: Portuguese, Spanish, German, French and Vietnamese are public while other locales remain unavailable", () => {
...
test("production manifest: only reviewed content in the six public languages is published", () => {
  ...
  assert.equal(contentRepository.listPublished().length, 593);
  assert.equal(contentRepository.listPublished().some(({ version }) => !["en", "pt-BR", "es", "de", "fr", "vi"].includes(version.locale)), false);
});
...
test("product localization batch 1: eighteen pages meet content, image, schema and publishing gates", () => {
  ...
  for (const content of localized) {
    ...
    assert.deepEqual(Object.keys(metadata.alternates?.languages ?? {}).sort(), ["de", "en", "es", "fr", "pt-BR", "vi", "x-default"].sort(), `${version.publicPath}: hreflang`);
    const graph = buildLocalizedSchemaGraph(content, siteUrl);
...
test("products hub has a complete six-language hreflang cluster", () => {
  ...
  assert.deepEqual(getLanguageSwitchOptions("products-hub", "pt-BR", contentRepository).map((option) => option.locale), ["en", "pt-BR", "es", "de", "fr", "vi"]);
});
...
test("language switch mapping: stable content ID resolves the matching localized slug", () => {
  ...
  assert.deepEqual(getLanguageSwitchOptions("dumbbells-category", "en", contentRepository).map((option) => option.locale), ["en", "pt-BR", "es", "de", "fr", "vi"]);
...
test("multilingual sitemap retains 135 English routes and adds all five localized route sets", () => {
  ...
  const englishUrls = urls.filter((url) => !portugueseUrls.includes(url) && !spanishUrls.includes(url) && !germanUrls.includes(url) && !frenchUrls.includes(url) && !vietnameseUrls.includes(url));
  ...
  assert.equal(new Set(urls).size, 605);
...
test("language sitemap contains all 605 public URLs with the complete products hub cluster", () => {
  const entries = localizedSitemapEntries();
  const english = entries.filter((entry) => !/^\/(?:pt|es|de|fr|vi)(?:\/|$)/.test(new URL(entry.url).pathname));
  ...
  assert.equal(new Set(entries.map((entry) => entry.url)).size, 605);
  for (const path of ["/products", "/pt/produtos", "/es/productos", "/de/produkte", "/fr/produits", "/vi/san-pham"]) {
