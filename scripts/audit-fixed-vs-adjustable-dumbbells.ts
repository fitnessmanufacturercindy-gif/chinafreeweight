...
const imageDirectory = "/assets/resources/fixed-vs-adjustable-dumbbells/";
...
const unsupportedClaims = /(?<![\p{L}\p{N}])(?:our customer|our client|client project|customer project|guaranteed capacity|guaranteed lead time|guaranteed MOQ|certified by IWF|certified by IPF|millions of drops|drop proof|zero maintenance|lifetime parts supply)(?![\p{L}\p{N}])/giu;
...
if (!entity) {
  ...
  if (new Set(heroes).size !== publishedLocales.length) failures.push("Every locale requires a distinct original hero image");
  ...
  for (const definition of publishedLocales) {
    ...
    if (version.title.length < (compact ? 20 : 32) || version.description.length < (compact ? 55 : 95) || version.h1.length < (compact ? 20 : 30)) failures.push(`${label}: metadata too thin`);
    if (version.body.length < 10 || version.body.filter((block) => block.heading).length < 10) failures.push(`${label}: section depth missing`);
    const h3Count = version.body.flatMap((block) => strings(block.data?.markdown)).join("\n").match(/^###\s+/gmu)?.length ?? 0;
    if (h3Count < 8) failures.push(`${label}: only ${h3Count} H3 sections`);
    ...
    if (!version.body.some((block) => block.type === "specifications")) failures.push(`${label}: comparison table missing`);
    if (!version.body.some((block) => block.type === "features")) failures.push(`${label}: sample checklist missing`);
    if (version.faq.length !== expectedFaqCount) failures.push(`${label}: expected ${expectedFaqCount} FAQs`);
    if (version.images.length !== expectedImageCount) failures.push(`${label}: expected ${expectedImageCount} images`);
    const expectedHero = `${imageDirectory}${locale.toLowerCase()}-fixed-adjustable-dumbbells.webp`;
    if (version.images[0]?.src !== expectedHero) failures.push(`${label}: incorrect localized hero ${version.images[0]?.src}`);
    const minimumLength = locale === "ko" ? 5_000 : locale === "ar" ? 7_000 : 6_000;
    ...
    if (/\uFFFD|鈥|脿|Ã|Lorem ipsum/gu.test(visible)) failures.push(`${label}: filler or mojibake`);
    ...
    for (const image of version.images) {
      ...
      if (image.alt.trim().length < 20 || (image.caption?.trim().length ?? 0) < 20) failures.push(`${label}: localized image text incomplete ${image.src}`);
    }

    if (version.internalLinks.length < (locale === "ar" ? 4 : 5)) failures.push(`${label}: too few repository links`);
    ...
    if (visibleLinks.length < 6) failures.push(`${label}: too few visible internal links`);
...
console.log(`Fixed-versus-adjustable dumbbell audit passed: ${publishedLocales.length} localized editions, distinct heroes, ${expectedFaqCount} FAQs, metadata, links and schemas.`);
