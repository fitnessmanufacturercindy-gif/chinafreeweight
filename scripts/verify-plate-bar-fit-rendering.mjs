...
const routes = [
  ["en", "/resources/olympic-plate-hole-barbell-sleeve-fit-guide"],
  ["pt-BR", "/pt/blog/furo-anilha-olimpica-encaixe-barra-50-mm"],
  ["es", "/es/blog/agujero-disco-olimpico-ajuste-barra-50-mm"],
  ["de", "/de/blog/hantelscheiben-bohrung-langhantel-50-mm-passung"],
  ["fr", "/fr/blog/alesage-disque-olympique-ajustement-barre-50-mm"],
  ["vi", "/vi/blog/lo-banh-ta-olympic-vua-ong-don-ta-50-mm"],
  ["sv", "/sv/blogg/viktskiva-haldiameter-passning-skivstang-50-mm"],
  ["it", "/it/blog/foro-disco-olimpico-compatibilita-bilanciere-50-mm"],
  ["nl", "/nl/blog/halterschijf-boring-passing-halterstang-50-mm"],
  ["ar", "/ar/blog/fathat-qurs-olimbi-tawafuq-qadib-50-mm"],
  ["ko", "/ko/blog/olympic-weight-plate-hole-barbell-sleeve-fit"],
  ["id", "/id/blog/lubang-plate-olimpiade-kecocokan-sleeve-barbel-50-mm"],
  ["pl", "/pl/blog/otwor-talerza-olimpijskiego-pasowanie-gryf-50-mm"],
];
...
const imageFragment = "/assets/resources/plate-bar-fit/";
const artifactDirectory = path.join(process.cwd(), ".artifacts", "plate-bar-fit-2026-09-06");
...
async function inspectPage(page, locale, route, viewportLabel, checkLinks = false) {
  ...
  if (!result.title || result.description.length < 50) failures.push(`${route}: title or description missing`);
  ...
  if (result.heroPath !== `${imageFragment}${locale.toLowerCase()}-plate-bar-fit.webp`) failures.push(`${route}: incorrect locale hero ${result.heroPath}`);
  if (result.h2Count < 5 || result.h3Count < 6 || result.faqCount !== 8) failures.push(`${route}: article hierarchy or FAQ missing`);
...
console.log(`Plate-and-bar fit rendering verified: ${routes.length}/13 pages, desktop/tablet/mobile, images, links, canonical, hreflang, schema and RTL.`);
