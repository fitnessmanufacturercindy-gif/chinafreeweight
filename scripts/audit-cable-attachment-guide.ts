...
const unsupportedClaimPatterns = /(?<![\p{L}\p{N}])(?:customer project|client project|our client|case study|certified|guaranteed lead time|guaranteed MOQ|sales increased|factory test proves)(?![\p{L}\p{N}])/giu;
...
if (!entity) {
  ...
  if (actualLocales.join("|") !== expectedLocales.join("|")) {
    failures.push(`locale coverage: expected ${expectedLocales.join(", ")}; found ${actualLocales.join(", ")}`);
  }
  ...
  for (const definition of publishedLocales) {
    ...
    if (version.reviewStatus !== "approved" || version.publishStatus !== "published") failures.push(`${label}: publication is not fully approved`);
    if (!version.publishedAt || !version.updatedAt || !version.author || !version.reviewedBy) failures.push(`${label}: editorial provenance is incomplete`);
    if (version.canonicalData.mode !== "self" || version.canonicalData.noindex) failures.push(`${label}: canonical/index state is invalid`);
    if (!version.hreflangData.include) failures.push(`${label}: hreflang is disabled`);
    if (!version.publicPath.startsWith(locale === "en" ? "/resources/" : `${definition.prefix}/`)) failures.push(`${label}: unexpected localized route ${version.publicPath}`);
    ...
    if (version.body.filter((block) => Boolean(block.heading)).length < 10) failures.push(`${label}: fewer than ten H2 sections`);
    ...
    if (!version.body.some((block) => block.data?.component === "quick-answer")) failures.push(`${label}: quick answer is missing`);
    if (!version.body.some((block) => block.data?.component === "definition")) failures.push(`${label}: definition is missing`);
    if (!version.body.some((block) => block.type === "specifications")) failures.push(`${label}: buyer specification table is missing`);
    if (!version.body.some((block) => block.type === "features")) failures.push(`${label}: buyer checklist is missing`);
    ...
    const minimumLinks = locale === "ar" ? 2 : 5;
    if (version.internalLinks.length < minimumLinks) failures.push(`${label}: only ${version.internalLinks.length} internal links`);
    for (const link of version.internalLinks) {
      if (!contentRepository.getPublishedVersion(link.targetContentId, locale)) failures.push(`${label}: unresolved internal target ${link.targetContentId}`);
    }
    for (const href of markdownLinks(version)) if (!allPaths.has(href)) failures.push(`${label}: unresolved visible link ${href}`);
...
console.log(`Cable attachment guide audit passed: ${publishedLocales.length} localized editions, ${expectedImageCount} optimized images and ${expectedFaqCount} FAQs each.`);
