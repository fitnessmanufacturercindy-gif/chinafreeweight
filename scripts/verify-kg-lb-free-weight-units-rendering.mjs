...
const routes = [
  ...
  [
    "pt-BR",
    "/pt/blog/halteres-anilhas-kg-lb-pedido-atacado"
  ],
  [
    "es",
    "/es/blog/mancuernas-discos-kg-lb-compra-mayorista"
  ],
  [
    "de",
    "/de/blog/kurzhanteln-hantelscheiben-kg-lb-bestellen"
  ],
  [
    "fr",
    "/fr/blog/halteres-disques-kg-lb-commande-gros"
  ],
  [
    "sv",
    "/sv/blogg/hantlar-viktskivor-kg-lb-inkop"
  ],
  [
    "vi",
    "/vi/blog/ta-don-dia-ta-kg-lb-dat-hang-si"
  ],
  [
    "ko",
    "/ko/blog/dumbbell-weight-plate-kg-lb-ordering"
  ],
  [
    "id",
    "/id/blog/dumbbell-plate-kg-lb-pesanan-grosir"
  ],
  [
    "it",
    "/it/blog/manubri-dischi-kg-lb-ordini-ingrosso"
  ],
  [
    "nl",
    "/nl/blog/dumbbells-halterschijven-kg-lb-inkoop"
  ],
  [
    "ar",
    "/ar/blog/dumbbell-aqras-kg-lb-talab-jumla"
  ],
  [
    "pl",
    "/pl/blog/hantle-obciazenia-kg-lb-zamowienia-hurtowe"
  ]
];
...
const imageFragment = "/assets/resources/kg-lb-units/";
const artifactDirectory = path.join(process.cwd(), ".artifacts", "kg-lb-units");
...
async function inspectPage(page, locale, route, viewportLabel, checkLinks = false) {
  ...
  const result = await page.evaluate(({ expectedHreflang, expectedAlternates, imageFragment, locale, route }) => {
    ...
    const headingClipped = headingBox && [...headingRange.getClientRects()].some((rect) => rect.left < headingBox.left - 1 || rect.right > headingBox.right + 1);
  ...
  if (result.heroPath !== `/assets/resources/kg-lb-units/${locale.toLowerCase()}-weight-assortment.webp`) failures.push(`${route}: incorrect locale hero ${result.heroPath}`);
  ...
  results.push({ locale, route, viewportLabel, ...result, visibleText: undefined, auditText: undefined });
  if (checkLinks) {
...
fs.writeFileSync(path.join(artifactDirectory, new URL(baseUrl).hostname === "127.0.0.1" ? "render-local.json" : "render-production.json"), JSON.stringify({baseUrl,checkedAt:new Date().toISOString(),results,failures},null,2));
...
console.log(`KG/LB free-weight rendering verified: ${routes.length}/13 pages, desktop/tablet/mobile, AVIF/WebP images, links, canonical, hreflang, schema and RTL.`);
