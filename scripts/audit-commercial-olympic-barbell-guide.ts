...
const entityId = "commercial-olympic-barbell-buying-guide";
...
if (!entity) {
  ...
  for (const definition of publishedLocales) {
    ...
    if (version.body.length < 14 || version.body.filter((block) => Boolean(block.heading)).length < 14) failures.push(`${label}: buyer sections missing`);
    ...
    const minimumVisibleLength = locale === "ko" ? 4800 : locale === "ar" ? 8200 : 9000;
...
console.log(`Commercial Olympic barbell guide audit passed: ${publishedLocales.length} localized editions, ${expectedImageCount} optimized images and ${expectedFaqCount} FAQs each.`);
