...
const entityId = "olympic-barbell-finish-coating-guide";
const siteUrl = "https://www.chinafreeweight.com";
const imageDirectory = "/assets/resources/barbell-finishes/";
...
const unsupportedClaims = /(?<![\p{L}\p{N}])(?:our customer|our client|client project|customer project|guaranteed capacity|guaranteed lead time|guaranteed MOQ|certified by IWF|certified by IPF|rust[- ]proof guarantee|zero maintenance|lifetime parts supply)(?![\p{L}\p{N}])/giu;
...
if (!entity) {
  ...
  for (const definition of publishedLocales) {
    ...
    if (version.title.length < (compact ? 20 : 28) || version.description.length < (compact ? 55 : 90) || version.h1.length < (compact ? 20 : 28)) failures.push(`${label}: metadata too thin`);
    ...
    const expectedHero = `${imageDirectory}${locale.toLowerCase()}-olympic-barbell-finishes.webp`;
    if (version.images[0]?.src !== expectedHero) failures.push(`${label}: incorrect localized hero ${version.images[0]?.src}`);
    const minimumLength = locale === "ko" ? 4_500 : locale === "ar" ? 5_500 : 5_800;
    ...
    if (version.internalLinks.length < 5) failures.push(`${label}: too few repository links`);
...
console.log(`Barbell-finish audit passed: ${publishedLocales.length} localized editions, distinct heroes, ${expectedFaqCount} FAQs, metadata, links and schemas.`);
