...
const imageDirectory = "/assets/resources/barbell-knurling/";
...
if (!entity) {
  ...
  for (const definition of locales) {
    ...
    if (!version.body.some((block) => block.type === "specifications")) failures.push(`${label}: decision table missing`);
    ...
    const expectedHero = `${imageDirectory}${locale.toLowerCase()}-barbell-knurling.webp`;
    if (version.images[0]?.src !== expectedHero) failures.push(`${label}: incorrect localized hero ${version.images[0]?.src}`);
    const minimumLength = locale === "ko" ? 4_200 : locale === "ar" ? 6_500 : locale === "vi" || locale === "sv" ? 7_500 : 8_000;
...
console.log(`Barbell-knurling audit passed: ${locales.length} localized editions, distinct heroes, 8 FAQs, metadata, links, assets and schemas.`);
