...
export function swedishVersionForProduct(profile: SwedishProductProfile): LocalizedContentVersion {
  ...
  const images:LocalizedImage[]=[source.image,"/assets/dumbbell-production.avif","/assets/resource-plate-finishing.webp"].map((src,imageIndex)=>({id:`bild-${imageIndex+1}`,src:swedishImagePath(src,profile.svSlug,imageIndex),alt:imageIndex===0?`${name} för kommersiell gymutrustning`:imageIndex===1?`Verklig produktion för ${name} hos PowerBaseFit`:`Yt- och kvalitetskontroll för ${name}`,caption:imageIndex===0?`Produktbild av ${name} från det befintliga PowerBaseFit-sortimentet.`:imageIndex===1?"Verkligt produktionsmoment som används för att diskutera process och kontrollpunkter.":"Verklig kontroll av yta och utförande före packning."}));
  const body=[
...
export function englishPathForSwedishProduct(profile: SwedishProductProfile) { const base=profile.category==="dumbbells"?"dumbbells":profile.category==="plates"?"weight-plates":"gym-accessories";return `/products/${base}/${profile.source.slug}`; }
