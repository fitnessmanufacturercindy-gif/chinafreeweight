...
import copyPrimary from "./barbell-finish-guide-copy.json";
import copyEu from "./barbell-finish-guide-copy-eu.json";
import copyApac from "./barbell-finish-guide-copy-apac.json";
...
function imagePath(locale: InternalLocale) {
  return `/assets/resources/barbell-finishes/${locale.toLowerCase()}-olympic-barbell-finishes.webp`;
}
...
function bodyFor(copy: GuideCopy, manifest?: ContentManifest): ContentBlock[] {
  const sections: ContentBlock[] = copy.sections.map((section, index) => ({
    id: `barbell-finish-section-${index + 1}`,
...
function versionFor(copy: GuideCopy, manifest: ContentManifest): LocalizedContentVersion {
  ...
  const internalLinks = [...targets, ...relatedTargets]
  ...
  return {
    ...
    faq: copy.faq.map(([question, answer], index) => ({ id: `barbell-finish-faq-${index + 1}`, question, answer })),
    author: { id: `powerbasefit-barbell-finish-${copy.locale}`, name: "PowerBaseFit", kind: "Organization", role: copy.eyebrow },
    reviewedBy: { id: `powerbasefit-barbell-finish-review-${copy.locale}`, name: "PowerBaseFit", kind: "Organization", role: copy.checklist.heading },
    schemaData: {
...
export const barbellFinishEnglishPost = {
  entityId,
  ...
  secondaryKeywords: [
    "hard chrome vs zinc barbell",
    "Cerakote barbell commercial gym",
    "barbell shaft and sleeve finish",
    "barbell coating corrosion test",
  ],
  searchIntent: "choose and specify Olympic barbell shaft and sleeve finishes for a commercial purchase",
...
export function withBarbellFinishGuide(manifest: ContentManifest): ContentManifest {
