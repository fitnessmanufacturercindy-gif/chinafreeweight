...
export async function GET(
  _request: Request,
  context: { params: Promise<{ encoded: string; filename: string }> }
) {
  ...
  if (!sourcePath.startsWith(`${publicRoot}${sep}`)) {
    return new NextResponse("Not found", { status: 404 });
  }
  ...
  try {
    ...
    return new NextResponse(bytes, {
      headers: {
        ...
        "Content-Type": contentTypes[extname(sourcePath).toLowerCase()] || "application/octet-stream"
      }
    });
  } catch {
    return new NextResponse("Not found", { status: 404 });
