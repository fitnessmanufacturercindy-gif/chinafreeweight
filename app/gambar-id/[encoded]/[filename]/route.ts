...
export async function GET(_request: Request, { params }: { params: Promise<{ encoded: string; filename: string }> }) {
  ...
  try {
    ...
    return new NextResponse(file, { headers: { "Content-Type": mime[path.extname(source).toLowerCase()] ?? "application/octet-stream", "Cache-Control": "public, max-age=31536000, immutable" } });
  } catch {
