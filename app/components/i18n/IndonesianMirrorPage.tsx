...
function ContactMirror({ version }: { version: Version }) {
  ...
  const factoryFacts = locale === "nl" ? [
    ...
    ["Profil perusahaan", "PowerBaseFit membantu buyer peralatan gym luar negeri dalam penawaran, pemilihan produk, dan komunikasi proyek OEM."],
    ["Basis manufaktur", "Zhengyang Road, Ningjin County, Dezhou, Shandong Province, China."],
    ["Produk utama", "Free weight: dumbbell, piring beban, bumper plate, rack, bench, dan aksesori gym."],
    ["Kemampuan OEM", "Logo, warna, finishing handle, detail produk, kemasan, dan program private label untuk buyer B2B."],
  ...
  return (
    <main className={`${styles.page} ${styles.contactPage}`} data-page-family="contact">
      <section className={styles.splitHero}>
        ...
        <div className={styles.heroMedia}><img src="/assets/project-dumbbell-zone.avif" alt={tr(locale, "Area free weight gym komersial PowerBaseFit", "Komercyjna strefa wolnych ciężarów PowerBaseFit", "Commerciële vrije-gewichtzone met PowerBaseFit-apparatuur")} /><div><strong>{tr(locale, "Respons dalam 12 jam", "Odpowiedź w ciągu 12 godzin", "Reactie binnen 12 uur")}</strong><span>{tr(locale, "Untuk importir, distributor, proyek gym, dan merek OEM", "Dla importerów, dystrybutorów, projektów siłowni i marek OEM", "Voor importeurs, distributeurs, sportschoolprojecten en OEM-merken")}</span></div></div>
      </section>
...
function ProductMirror({ content }: { content: PublishedContent }) {
  ...
  const featureItems = localized ? version.body.flatMap(dataItems).slice(0, 6) : source?.features ?? source?.details ?? version.body.flatMap(dataItems).slice(0, 6);
  const oemItems = locale === "nl" ? ["Logo en gewichtsaanduiding", "Kleur of afwerking", "Etiket en exportdoos", "Planning van exportpallets"] : locale === "pl" ? ["Logo i oznaczenia masy", "Kolor lub wykończenie", "Etykieta oraz karton", "Planowanie palety eksportowej"] : source?.oemOptions ?? ["Logo dan penandaan berat", "Warna atau finishing", "Label dan karton", "Perencanaan pallet ekspor"];
  const productType = localized ? version.schemaData.category : source?.type ?? version.schemaData.category;
  const productRange = localized ? version.schemaData.specifications?.find((item) => item.name === (locale === "nl" ? "Bereik" : "Zakres"))?.value : source?.range;
  return (
    <main className={`${styles.page} ${styles.productPage}`} data-page-family="product-detail">
      <section className={styles.productHero}>
        <div className={styles.productCopy}><a href={version.schemaData.breadcrumbs?.at(-2)?.path ?? tr(locale, "/id/produk", "/pl/produkty", "/nl/producten")}>← {tr(locale, "Kembali ke kategori", "Wróć do kategorii", "Terug naar de categorie")}</a><span>{productType}</span><h1>{version.h1}</h1><p>{version.description}</p><div className={styles.specRail}><div><small>{tr(locale, "Rentang", "Zakres", "Bereik")}</small><strong>{productRange ?? tr(locale, "Sesuai konfigurasi", "Według zatwierdzonej konfiguracji", "Volgens goedgekeurde configuratie")}</strong></div><div><small>{tr(locale, "Jenis produk", "Rodzaj produktu", "Producttype")}</small><strong>{productType}</strong></div><div><small>{tr(locale, "Fokus buyer", "Odbiorcy", "Doelgroep")}</small><strong>{tr(locale, "Gym, distributor, dan merek OEM", "Siłownie, dystrybutorzy i marki OEM", "Sportscholen, distributeurs en OEM-merken")}</strong></div></div><a className={styles.primaryButton} href={tr(locale, "/id/kontak", "/pl/kontakt", "/nl/contact")}>{tr(locale, "Minta penawaran", "Poproś o wycenę", "Offerte aanvragen")} <ArrowRight size={20} /></a></div>
      ...
      <section className={styles.twoColumn}><div><span>{tr(locale, "Aplikasi", "Zastosowanie", "Toepassing")}</span><h2>{tr(locale, "Penggunaan dan posisi produk", "Sposób użycia i pozycjonowanie produktu", "Gebruik en positionering van het product")}</h2><p>{localized ? textBlock(version.body[5], version.description) : source?.buyerNotes ?? textBlock(version.body[5], version.description)}</p><ul>{(locale === "nl" ? ["Commerciële sportscholen", "Distributeurs en importeurs", "Private-labelprojecten"] : locale === "pl" ? ["Siłownie komercyjne", "Dystrybutorzy i importerzy", "Projekty marki własnej"] : source?.applications ?? ["Gym komersial", "Distributor", "Proyek merek sendiri"]).map((item) => <li key={item}>{item}</li>)}</ul></div><div><span>{tr(locale, "OEM dan private label", "OEM i marka własna", "OEM en private label")}</span><h2>{tr(locale, "Opsi merek untuk pasar Indonesia", "Opcje marki dla rynku polskiego", "Merkopties voor de Nederlandse markt")}</h2><ul>{oemItems.map((item) => <li key={item}><ShieldCheck />{item}</li>)}</ul></div></section>
  ...
  return (
    <main className={`${styles.page} ${styles.resourcesPage}`} data-page-family="resources">
      <section className={styles.resourceHero}>
        <div><span>{tr(locale, "Pusat pengetahuan free weight", "Centrum wiedzy o wolnych ciężarach", "Kenniscentrum voor vrije gewichten")}</span><h1>{version.h1}</h1><p>{version.description}</p></div>
