...
import type { SeoUrl } from "../seo-data";
...
export function urlsetXml(items: SeoUrl[]) {
...
export function languageSitemapXml(items: SeoUrl[]) {
  const today = new Date().toISOString();
  const urls = items
    .map((item) => {
      const links = Object.entries(item.alternates || {})
        .map(
          ([lang, path]) =>
