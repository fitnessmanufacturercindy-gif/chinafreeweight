...
function assertCustomerVisibleContent(body: string, slug: string) {
  ...
  const hasDefinition = /^##\s+.+(?:Overview|Manufacturing|Customization|Branding|OEM).*$[\s\S]{80,}/im.test(visible);
...
function renderMarkdown(body: string) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (text.startsWith("*") && text.endsWith("*") && !text.startsWith("**")) {
      elements.push(<figcaption key={index}>{text.slice(1, -1)}</figcaption>);
      continue;
    }
